...
export class ShoppingService {
  ...
  private dataDrawnCo2TimeSerie!: Product[];
  ...
  public formatDate(data: Product[]):Product[] {
    let newData: Product[] = [];
    data.forEach(ms => {
      if (typeof ms.date === 'string' || typeof ms.date === 'number') {
        newData.push({co2: ms.co2, date: new Date(ms.date), name: ms.name, weight: ms.weight, co2ByKg: ms.co2ByKg});
      } else {
        newData.push({co2: ms.co2, date: ms.date, name: ms.name, weight: ms.weight, co2ByKg: ms.co2ByKg});
      }
    });
    return newData;
  }

  public scaleXYDomain(data: Product[], x: any, y: any) {
  ...
  public reducePointsCo2TimeSerie(data: Product[]): Product[] { // very approximative, need to refactor this function
    let i = 0;
    let reducedData: Product[] = [data[0]];
  ...
  public buildTooltip(chartProps: any) {
    ...
    function onMouseMove(event: any) {
      ...
      const yAccessor = (d: Product) => d.co2;
      const xAccessor = (d: Product) => d.date;
  
      const getDistanceFromHoveredDate = (d: Product) => Math.abs((xAccessor(d) as unknown as number) - hoveredDate);
      const getDistanceFromHoveredCo2 = (d: Product) => Math.abs((yAccessor(d) as unknown as number) - hoveredCo2);
  ...
  public fillIndicators(dataSum: Product[]): void {
