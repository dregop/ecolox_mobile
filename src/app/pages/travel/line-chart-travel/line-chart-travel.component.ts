...
@Component({
  selector: 'line-chart-travel',
  templateUrl: './line-chart-travel.component.html',
  styleUrls: ['./line-chart-travel.component.scss']
})
export class LineChartTravelComponent implements OnInit {

  @ViewChild('chart') chartElement!: ElementRef;

  parseDate = d3.timeParse('%d-%m-%Y');

  public dataSumDbExtCo2TimeSerieFiltered: Travel[] = [];
  public dataDrawnCo2TimeSerie: Travel[] = [];
  public dataThreshold: Travel[] = [];
  ...
  @Input() public dataDb: Travel[] = [];
  constructor(public toastService: ToastService, private userService: UserService, private travelService: TravelService) {
  ...
  ngOnInit(): void {
    ...
    this.travelService.$browserName.subscribe((browser) => {
      this.browserName = browser;
    });

    this.travelService.setD3Locale(); // initiate date for x graph 
    ...
    treasholdButton?.addEventListener('click', () => {
      if (treasholdButton.className.includes('activated')) {
        ...
        if (!dayButton?.className.includes('activated')) {
          this.travelService.scaleXYDomain(sumAllData, this.chartProps.x, this.chartProps.y);
          this.zoomTransform();
          this.chartProps.svgBox.on('.zoom', null);
        }
    ...
    Array.from(buttons as any).forEach((button: any) => {
      (button as HTMLInputElement).addEventListener(('click'), () => {
        ...
        Array.from(this.daysOfWeek).forEach((day: any) => {
          j++;
          if ((j > new Date().getDay() % 7 && day && this.travelService.getWeek(new Date()) === this.travelService.getWeek(selectedDate)) || this.dataSumDbExtCo2TimeSerieFiltered.length === 0) {
    ...
    if (this.daysOfWeek && this.daysOfWeek.length > 0) {
      for (let i = 0; i < this.daysOfWeek.length; i++) {
        ...
        this.daysOfWeek[i].addEventListener('click', (e) => {
          ...
          const dataDayOfWeek = this.dataDb.filter(d => new Date(d.date).getDay() % 7 === (i + 1) % 7 && this.travelService.getWeek(new Date(d.date)) === this.travelService.getWeek(selectedDate) && new Date(d.date).getFullYear() === this.currentSelectedYear);
  ...
  private buildChart() {
    ...
    this.dataDrawnCo2TimeSerie = this.travelService.reducePointsCo2TimeSerie(this.dataDb);
   ...
   .call(d3.axisLeft(y).ticks(5).tickPadding(height / 80).tickSize(-15000));
    ...
    // ZOOM
    this.zoom = d3.zoom()
    .on('zoom', (event) => this.travelService.chartZoom(event, this.chartProps, gx, gy, xAxis, yAxis, this.valueslines))
    .scaleExtent([1, 20]);
  ...
  private updateChart() {
    ...
    this.dataDrawnCo2TimeSerie = this.travelService.reducePointsCo2TimeSerie(this.dataDrawnCo2TimeSerie);
    ...
    if (time_range) {
      ...
      start_date.getMonth() === end_date.getMonth()) {
        const formatDate = this.travelService.d3Locale.format("%A %d %B");
